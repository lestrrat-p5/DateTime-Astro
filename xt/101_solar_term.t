...
sub cmpDate{
 ...
 if ($fullDt->ymd ne  $onlyDt) {
   if ($fullDt->hour() >=23) {$fullDt->add( {days=> 1});}
   if ($fullDt->hour() <=0) {$fullDt->add( {days=> -1});}
  }
